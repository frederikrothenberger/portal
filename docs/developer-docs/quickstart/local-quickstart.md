...
2.  Navigate to the URL output by the `dfx deploy` command in the previous step. It's the one after `Frontend canister via browser`, in our case that's <http://127.0.0.1:4973/?canisterId=ryjl3-tyaaa-aaaaa-aaaba-cai>.
